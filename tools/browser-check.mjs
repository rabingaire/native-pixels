...
try{
  ...
  const loadPage = async file => {
    ...
    const loaded = new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => reject(Error(`Page load timeout: ${file}`)), 15000);
    });
    try {
      await Promise.all([call('Page.navigate',{url:pathToFileURL(path.join(root,file)).href}), loaded]);
    } finally {
      clearTimeout(timer);
  ...
  await evaluate('document.querySelector("#chapter-search").value="shader";document.querySelector("#chapter-search").dispatchEvent(new Event("input"))');
  ...
  await screenshot('code-dark.png',1440,1000,'.code-block');
  await evaluate('document.documentElement.dataset.theme="light"');
  await screenshot('code-light.png',1440,1000,'.code-block');
  await screenshot('code-mobile.png',390,900,'.code-block');
  const tokenColor = () => evaluate('getComputedStyle(document.querySelector("[data-highlighted] span")).color');
  const lightToken = await tokenColor();
  await evaluate('document.documentElement.dataset.theme="dark"');
  assert.notEqual(await tokenColor(), lightToken, 'Code token colors follow the selected theme');
  await call('Emulation.setEmulatedMedia',{media:'print'});
  assert.equal(await tokenColor(), 'rgb(17, 17, 17)', 'Printed code stays legible when dark mode is selected');
  ...
  await navigate('chapters/18-from-main-to-pixels.html');
  await evaluate('document.querySelector(".object-diagram").scrollIntoView({behavior:"instant"})');
  await screenshot('chapter-18-diagram.png',1440,900,'.object-diagram');
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:900,deviceScaleFactor:1,mobile:true});
  await evaluate('document.querySelector(".object-diagram").scrollIntoView({behavior:"instant"})');
  await screenshot('chapter-18-diagram-mobile.png',390,900,'.object-diagram');
  ...
  // The protocol's load event works with scripts disabled and ensures that DOM
  // node IDs belong to the new document, rather than the page being replaced.
  await loadPage('chapters/18-from-main-to-pixels.html');
  ...
  const staticTokens=await call('DOM.querySelectorAll',{nodeId:documentNode.root.nodeId,selector:'pre code[data-highlighted] span[style]'});
  assert(staticTokens.nodeIds.length>0,'Syntax highlighting is available without JavaScript');
  ...
  const report=JSON.parse(fs.readFileSync('verification.json','utf8'));report.browser={status:'passed',pages:pages.length,viewports,themes:['light','dark'],pageViewportChecks:results.length,offlineFileURLs:true,noJavaScriptReading:true,staticSyntaxHighlighting:true,codeThemeColors:true,printCodeContrast:true,theme:true,chapterSearch:true,coordinateInteraction:true,staticSectionNavigation:true,consistentPageGeometry:true,sidebarHoverGeometry:true,resumeReading:true,expandedMobileSource:true,svgLabelsWithinViewBox:true,imagesLoaded:true,optionalChallenges:true,screenshots:['code-light.png','code-dark.png','code-mobile.png','home-desktop.png','home-mobile.png','chapter-dark.png','chapter-04-desktop.png','chapter-18-diagram.png','chapter-18-diagram-mobile.png','chapter-challenges-mobile.png','chapter-challenges-desktop.png','home-hover.png','contents-hover.png','chapter-hover.png','reference-hover.png','home-dark.png']};report.updated=new Date().toISOString();fs.writeFileSync('verification.json',JSON.stringify(report,null,2)+'\n');
