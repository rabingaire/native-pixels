...
capture_encode :: proc(r: ^State, encoder: wgpu.CommandEncoder, texture: wgpu.Texture) -> Capture {
	...
	dst := wgpu.TexelCopyBufferInfo {
		buffer = c.buffer,
		layout = {bytesPerRow = c.stride, rowsPerImage = c.height},
	}
...
capture_save :: proc(r: ^State, capture: ^Capture) {
	...
	for y in 0 ..< int(capture.height) {
		for x in 0 ..< int(capture.width) {
			...
			if r.gpu.config.format == .BGRA8UnormSrgb {
				rgba[dst + 0] =
					data[src + 2]; rgba[dst + 1] = data[src + 1]; rgba[dst + 2] = data[src + 0]
			} else {
				rgba[dst + 0] =
					data[src + 0]; rgba[dst + 1] = data[src + 1]; rgba[dst + 2] = data[src + 2]
			}
	...
	assert(
		stbi.write_png(
			filename,
			c.int(capture.width),
			c.int(capture.height),
			4,
			raw_data(rgba),
			c.int(capture.width * 4),
		) !=
		0,
	)
