...
import { esc, code, note, section, table } from '../authoring/html.mjs';
...
for (const c of chapters) {
  ...
  const changeTable = modifications.length ? table(['File','Operation'], modifications.map(f=>[`<code>${f}</code>`,snapshot[f]===undefined?'Remove: replacement lives in the new package': previous[f]===undefined?'Add this file':'Replace the changed portions shown below; preserve all other code'])) : '<p>No implementation changes. This checkpoint retains the previous complete runnable source.</p>';
  let diff = '';
  ...
  const snapshotHTML = Object.entries(snapshot).map(([f,source])=>`<details class="source-file"><summary>${esc(f)} <span>${source.trimEnd().split('\n').length} lines</span></summary><p><a download href="../checkpoints/${n}/${f}">Download this checkpoint’s ${f}</a></p><figure class="code-block"><figcaption>checkpoints/${n}/${f} · complete file</figcaption><pre><code class="language-${f.endsWith('.wgsl')?'wgsl':'odin'}" data-source="checkpoints/${n}/${f}">${esc(source)}</code></pre></figure></details>`).join('');
  const body = learningGuide(c.number) + c.body;
  const proseWords = body.replace(/<pre>[\s\S]*?<\/pre>/g,'').replace(/<[^>]+>/g,' ').split(/\s+/).length;
  page(chapterPath(c), c.title, `<article class="chapter"><header class="chapter-header"><span class="chapter-display-number" aria-hidden="true">${c.number}</span><p class="eyebrow">${c.part} <span>Chapter ${n} / ${chapters.length}</span></p><h1>${c.title}</h1><p class="deck">${c.deck}</p><p class="reading-meta">${Math.ceil(proseWords/180)} min read + implementation · checkpoint ${n}</p></header>${note('At the end of this chapter',c.outcome,'outcome')}<nav class="on-this-page" aria-label="On this page" hidden></nav>${body}${section('What we have now',`<p>${c.outcome}</p><p>Before moving on, run this checkpoint and compare it with the stated result. If it differs, use the symptom table above or the complete files below to locate the missing step.</p><p>Run <code>odin run checkpoints/${n}/src</code> from the book root, with the SDL linker flags for your platform. <a href="../appendix/build.html">Dependency and build instructions →</a></p>`)}${challenges(c.number)}${section('Exact code changes and runnable checkpoint',changeTable + (diff?`<details class="change-record"><summary>Exact patch from the preceding checkpoint</summary><p><a download href="../checkpoints/${n}/changes.patch">Download changes.patch</a> · inspect or apply with <code>patch -p1</code> from your own project directory at the previous checkpoint.</p>${code(diff,'diff')}</details>`:'') + `<p>Complete end-of-chapter files follow. These are already present in <code>checkpoints/${n}/</code>; no assembly from excerpts is required.</p>${c.number>=19?`<p>Binary assets are separate from the text patch: <a href="../checkpoints/${n}/assets/room.png" download>room.png</a> · <a href="../checkpoints/${n}/assets/player.png" download>player.png</a>${c.number>=23?` · <a href="../checkpoints/${n}/assets/player-walk.png" download>player-walk.png</a>`:''}. Copy them into assets/ in this checkpoint.</p>`:''}${snapshotHTML}`)}${nav(c)}</article>`, {description:c.deck});
}
