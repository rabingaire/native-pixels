...
for(const name of pages){
  ...
  for (const m of html.matchAll(/<figure class="code-block"><figcaption>([\s\S]*?)<\/figcaption>/g)) {
    assert(!/::\s*(?:$|[·—])/.test(decode(m[1])), `${name}: unfinished declaration in snippet caption`);
  }
