...
Open [index.html](index.html) directly in a browser. No server, package installation, CDN, or build step is needed to read it. [Contents](contents.html) lists all 23 chapters and six reference appendices.
...
Independent runnable snapshots run from `checkpoints/01` through `checkpoints/23`. The original 18-checkpoint untextured foundation is preserved. Chapters 19–23 add texture uploads, the renderer/game package boundary, collision, F1 diagnostics, and four-direction animation. Readers build one `native-pixels-game` directory from scratch. Each chapter establishes its starting state, teaches the new concepts, and includes exact changes, complete source, build commands, expected results, and required artwork. Chapter 1 includes platform setup; chapters 21 and 23 include complete test files. Optional source snapshots are described in the source-code appendix. All 23 chapters include optional extension challenges with acceptance criteria. These are independent of the main path: skipping every challenge still produces the complete final game.
...
Authoring requires Node 22+, `npm ci`, and `odinfmt` from the
[OLS dev-2026-08 release](https://github.com/DanielGavin/ols/releases/tag/dev-2026-08).
Put the formatter on PATH as `odinfmt`, set `ODINFMT` to its absolute path, or
extract the platform-named binary into `build/formatter/`. The publishing workflow
installs the pinned Linux binary and checks its SHA-256 before generating the book.
These tools are only needed for the developer commands below:

```sh
node tools/generate.mjs
node tools/verify.mjs
node tools/verify.mjs --native
node tools/reader-check.mjs --native
...
`node tools/browser-check.mjs` uses installed Google Chrome over its debugging protocol to check all pages at seven widths (360, 390, 768, 1024, 1280, 1440, and 1680 pixels) in light and dark themes, offline navigation, theme, search, coordinate interaction, and reading with scripts disabled. `BOOK_CHROME` can select a different Chrome executable. Browser profiles are isolated under a temporary directory; screenshots go under ignored `build/screenshots/`.

`node tools/reader-check.mjs --native` reconstructs all 23 projects from the displayed HTML code and attached images, builds each in its own directory, and runs the in-book tests at chapters 21–23. It does not import the source-snapshot generator or launch a graphics window.

Without `--native`, the reader check reconstructs the projects and verifies that each chapter’s change table accounts for every application file added, replaced, or removed since the preceding chapter. The site build runs this check automatically. Static reconstruction results are recorded separately so they preserve the dated native-build evidence.
...
Edit `authoring/source.mjs` for foundation application changes; `authoring/room-source/` and `authoring/room-source.mjs` own the extension and its snapshot migrations. The chapter dependency review is in `EDITORIAL.md`. Shared installation and prerequisite lessons live in `authoring/setup.mjs` and `foundations.mjs`; `reader-tests.mjs` publishes tests with reader-relative imports. Prose lives in `authoring/chapters.mjs`, `room-chapters.mjs`, `animation-chapter.mjs`, and `appendices.mjs`; beginner walkthroughs and optional extension challenges live in `authoring/learning.mjs`; the homepage is `home.mjs`. Regenerate afterward. Generated source, checkpoints, full-source listings, and HTML should not be edited independently. PNGs and prompts under `project/assets/` are original source assets, not generated by the website builder. Reader styling and optional interaction live in `assets/style.css` and `assets/book.js`.

Generation applies `odinfmt.json` after assembling each snapshot, keeping the
authoring templates' exact replacement anchors intact. Published Odin uses tabs
displayed at four columns, aligned fields, and a 100-column formatting target.
Shiki renders VS Code's Light+/Dark+ syntax colors into the HTML during generation;
highlighting works offline and without JavaScript. Copy buttons preserve the exact
formatted source. `.editorconfig` keeps standalone Odin edits consistent.
...
The production address is `https://rabingaire.com.np/native-pixels/`; the `github.io` address redirects to this account-level custom domain. Edit `authoring/site.mjs` when changing the hostname or repository path and regenerate. Every reading page has its own title, description, canonical URL, social preview, and structured book metadata. `sitemap.xml` lists the 31 canonical reading URLs; the custom 404 page is excluded from indexing. Relative navigation continues to work offline and under the GitHub Pages project path.
